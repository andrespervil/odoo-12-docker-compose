...
class informacion (models.Model):
    ...
    def _actualiza_sexo(self, rexistro):
    ...
    def _mes_castelan(self):
        # O idioma é o configurado en locale na máquina de odoo podemos cambialo con locale.setlocale
        locale.setlocale(locale.LC_TIME,'es_ES.utf8') # o locale ten que estar instalado na máquina onde se executa odoo
        for rexistro in self:
            rexistro.mes_castelan = rexistro.data_hora.strftime("%B")
    ...
    def _mes_galego(self):
        ...
        locale.setlocale(locale.LC_TIME,
                         'gl_ES.utf8')  # o locale ten que estar instalado na máquina onde se executa odoo
    ...
    # Engadimos na vista  buttons no header. O name do button é o nome da función

    def actualiza_hora_usuario(self,obxeto_rexistro):  # Convertimos a hora UTC a hora do usuario
    ...
    def actualiza_hora_usuario_dende_boton_e_apidepends(self):# Esta función é chamada dende un boton do xml
        self.actualiza_hora_usuario(self)  # leva self como parametro por que actualiza_hora ten 2 parametros
        # porque usamos tamén actualiza_hora_usuario dende outro modelo e lle pasamos como parametro o rexistro

    @api.depends('data_hora')
    def _hora_usuario(self):
        self.actualiza_hora_usuario_dende_boton_e_apidepends()

    def actualiza_hora_actual(self): # Esta función é chamada dende un boton do xml
        for rexistro in self:
            rexistro.hora_actual = fields.Datetime.now().strftime("%H:%M:%S")

    @api.depends('data_hora')
    def _hora_actual(self):
        self.actualiza_hora_actual()

    def ver_contexto(self): # Esta función é chamada dende un boton do xml
    ...
    def data_hora_local(self): # Esta función é chamada dende un boton do xml
